...
class Trainer(object):
    ...
    def translate(self, args, s):
        # construnct example(dataloader.Batch):

        d = data_builder._format_to_bert_translate(args, s)
...
#       d= torch.load("/Users/michen/src/BertSum/bert_data/cnndm.test.0.bert.pt")
#       for k in d[0]:
#            print (k, str(d[0][k]))
#
#        print("------------------------------------------------------------------------------")

        x=[d[0]["src"], d[0]["labels"], d[0]["segs"], d[0]["clss"], d[0]["src_txt"], d[0]["tgt_txt"] ]

        example = data_loader.Batch([x], 'cpu', True)
        res= self.example_api(example, args.top_n_sentences)
        print("processing top_n_sentences"+str(args.top_n_sentences))
        #for summary in res["predicted_summary"]:
        #    for s in summary.split('<q>'):
        #        print("---"+s)
        #print("[predicted_summary]:"+res["predicted_summary"][0].split('<q>'))
        return res
    
    def example_api(self, example, top_n_sentences=3, device='cpu', cal_lead=False, cal_oracle=False):
        """ 
        ...
        for i, idx in enumerate(selected_ids):
            ...
            for j in selected_ids[i][:len(example.src_str[i])]:
                ...
                print("len pred ", len(_pred) )
                print("top_n_sentences ", top_n_sentences)
                # len(_pred) == 3 means that we limit sentences to top top_n_sentences
                if ((not cal_oracle) and (not self.args.recall_eval) and len(_pred) == top_n_sentences):
                    break
        ...
        results = {'src': source_article, 'tgt': pred}
